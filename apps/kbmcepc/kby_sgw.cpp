			...
			else
			{
				...
				switch(act_type)
				{
					case 1:
						...
						if(retval == 0)
						{
							TRACE(cout<<"Info: connection closed by MME, erasing all data "<<errno<<" retval "<<retval<<endl;)
						...
						if(retval < 0)
						{
							cout<<"ERROR: read packet from MME "<<errno<<" retval "<<retval<<endl;
							exit(-1);
						}
					...
					case 3:
						...
						pkt.clear_pkt();
						retval = mtcp_read(mctx, cur_fd, data,BUF_SIZE);
						if(retval <= 0)//being liberal, pgw does not close conn on its own...
						{
							cout<<"Error: pgw read packet "<<errno<<" retval "<<retval<<endl;
							exit(-1);
						}
						memcpy(&pkt_len, data, sizeof(int));
						dataptr = data+sizeof(int);
					...
					case 4:
						...
						if(retval <= 0)
						{
							cout<<"Error: pgw read packet detach "<<errno<<" retval "<<retval<<endl;
							exit(-1);
						}
						memcpy(&pkt_len, data, sizeof(int));
						dataptr = data+sizeof(int);
