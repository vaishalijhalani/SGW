...
void *action(void *arg)
{
	...
	*/
	...
	TRACE2(cout << "in thread " << threadID << endl;)
	...
	*/
	...
	struct sockaddr_in from_mme;
	struct sockaddr_in c_addr, a_addr, bserver;
	socklen_t fromlen = sizeof(from_mme);
	int port_mme;
	set<int> srcr;
	struct cadata cd;
	int buffer = 0;
	...
	struct hostent *c_ip;
	int acfd, portno, n, numev, i, rev_fd, ccfd, pgw_fd;
	...
	*/
    ...
    /*
		epoll create and event registration
	*/
   	...
   	while( 1 )
   	{
		...
		for( i = 0; i < numev; i++)
		{
			...
			{							
					...
					bzero((char *) &from_mme, sizeof(from_mme) );
					rev_fd = rev[i].data.fd;
					...
					if(rev_fd == lsfd)
					{
						while(1)
						{
								...
								pkt.clear_pkt();
						        retval = recvfrom(rev_fd, pkt.data, BUF_SIZE, 0, (struct sockaddr *) &from_mme, &fromlen);
						        if (retval < 0) 
						        {
						        ...
						        port_mme = ntohs(from_mme.sin_port);
						        TRACE2(cout << port_mme << endl;)
						        pkt.data_ptr = 0;
								pkt.len = retval;
								pkt.extract_gtp_hdr();
								...
								pthrehss_handleautoinforeq: response sentad_mutex_unlock(&s5id_mux);
								...
								pgw_port.insert(make_pair(port_mme, pgdata));
							...
							{//attach 4 from mme
								...
								a_addr.sin_port = htons(port_mme);

								n = sendto(rev_fd, pkt.data, pkt.len, 0,(const struct sockaddr *)&a_addr,sizeof(a_addr));	
								if(n < 0){
			        ...
			        else if(srcr.find(rev[i].data.fd) != srcr.end())
			        {
								...
								pkt.clear_pkt();
								bzero((char *) &from_mme, sizeof(from_mme) );
						        n = recvfrom(retfd, pkt.data, BUF_SIZE, 0, (struct sockaddr *) &from_mme, &fromlen);
						        if ( n < 0) {
						        	//break;
						           cout<<"Error : Read Error "<<'\n';
						           exit(-1);
