...
#define MAX_THREADS 1 // This constant define number of threads available
		...
		for(int i = 0; i < numevents; ++i)
		{
			...
			*/
			...
			{	
				...
				handle_ran_accept(mctx, epollfd, ran_listen_fd);
			...
			else
			{
				...
				switch(act_type)
				{
					...
					case 1:
						...
						if(retval == 0)
						{
							TRACE(cout<<"Info: connection closed by RAN, erasing all data "<<errno<<" retval "<<retval<<endl;)
						...
						if(retval < 0)
						{
							TRACE(cout<<"ERROR: read packet from ran "<<errno<<" retval:"<<retval<<endl;)
							exit(-1);
						}
						...
						if (pkt.s1ap_hdr.mme_s1ap_ue_id == 0) {
							...
							handle_hss_connect(mctx, epollfd, cur_fd, pkt, mme_s1ap_ue_id);
						...
						if(pkt.s1ap_hdr.msg_type == 2)
						{
							...
							send_ran_attach_two(mctx, epollfd, cur_fd, pkt);
						...
						if(pkt.s1ap_hdr.msg_type == 3)
						{
							...
							//send pgw ip/port to sgw 		TODO
							handle_sgw_connect(mctx, epollfd, cur_fd, pkt, mme_s1ap_ue_id );
						...
						if(pkt.s1ap_hdr.msg_type == 4)
						{
							...
							send_sgw_afour(mctx, epollfd, cur_fd, fddata, pkt);
						...
						if(pkt.s1ap_hdr.msg_type == 5)
						{
							...
							fddata.msui = mme_s1ap_ue_id;
							send_sgw_detach(mctx, epollfd, cur_fd, fddata, pkt);
					...
					case 2:
						...
						{
							send_request_hss(mctx, epollfd, cur_fd, fddata);
					...
					case 3:
						...
						if(retval < 0)
						{
							cout<<"Error hss epoll read delete from epoll"<<endl;
							exit(-1);
						}

						pkt.clear_pkt();
						retval = mtcp_read(mctx, cur_fd, data,BUF_SIZE);
						if(retval <= 0)//being liberal, hss doesnot close conn...
						{
							cout<<"Error: hsss read packet "<<errno<<" retval "<<retval <<endl;
							exit(-1);
						}
						memcpy(&pkt_len, data, sizeof(int));
						dataptr = data+sizeof(int);
						...
						send_ran_attach_one(mctx, epollfd, ran_fd, pkt);
					...
					case 4:
						...
						if(return_events[i].events & MTCP_EPOLLOUT)
						{
							send_request_sgw_athree(mctx, epollfd, cur_fd, fddata);
					...
					case 5:
						...
						pkt.clear_pkt();
						retval = mtcp_read(mctx, cur_fd, data,BUF_SIZE);
						if(retval <= 0)//being liberal, sgw does not close conn
						{
							cout<<"Error: sgw read packet a3 "<<errno<< "retval "<<retval<<endl;
							exit(-1);
						}
						memcpy(&pkt_len, data, sizeof(int));
						dataptr = data+sizeof(int);
					...
					case 6:
						...
						pkt.clear_pkt();
						retval = mtcp_read(mctx, cur_fd, data,BUF_SIZE);
						if(retval <= 0)//being liberal
						{
							cout<<"Error: sgw read packet a4"<<errno<< "retval "<<retval<<endl;
							exit(-1);
						}
						memcpy(&pkt_len, data, sizeof(int));
						dataptr = data+sizeof(int);
						...
						fdmap.insert(make_pair(ran_fd,fddata));
						munlock(fdlock);
						TRACE(cout<<"Attach 4 sent to RAN"<<endl;)
						//go to act_type case 1, next header = 5
					break;//case 6
					...
					case 7:
						...
						pkt.clear_pkt();
						retval = mtcp_read(mctx, cur_fd, data,BUF_SIZE);
						if(retval <= 0)//being liberal, and careless...
						{
						...
						munlock(fdlock);
						//not needed , do when further transactions on same connections...
						/*
						epevent.data.sockid = ran_fd;
						epevent.events = MTCP_EPOLLIN;
...
void handle_ran_accept(mctx_t mctx, int epollfd, int ran_listen_fd)
{
...
void handle_hss_connect(mctx_t mctx, int epollfd, int cur_fd, Packet pkt, int mme_s1ap_ue_id)
{
...
void send_request_hss(mctx_t mctx, int epollfd, int cur_fd, struct mdata fddata)
{
	int returnval;
	struct mtcp_epoll_event epevent;
...
void send_ran_attach_one(mctx_t mctx, int epollfd, int cur_fd, Packet pkt)
{	
	int retval;
	struct mdata fddata;
	struct mtcp_epoll_event epevent;
...
void send_ran_attach_two(mctx_t mctx, int epollfd, int cur_fd, Packet pkt)
{
	int retval;
	struct mdata fddata;
	struct mtcp_epoll_event epevent;
...
void handle_sgw_connect(mctx_t mctx, int epollfd, int cur_fd, Packet pkt, int msui)
{
...
void send_request_sgw_athree(mctx_t mctx, int epollfd, int cur_fd, struct mdata fddata)
{
	int returnval;	
	struct mtcp_epoll_event epevent;
	returnval = mtcp_write(mctx, cur_fd, fddata.buf, fddata.buflen);
...
void send_sgw_afour(mctx_t mctx, int epollfd, int cur_fd, struct mdata fddata, Packet pkt)
{
	int returnval;
...
void send_sgw_detach(mctx_t mctx, int epollfd, int cur_fd, struct mdata fddata, Packet pkt)
{
	int returnval;
	int sgw_fd = fddata.initial_fd;
	struct mtcp_epoll_event epevent;
